...
def render_agents_md(project: Path, slug: str, cfg: dict, agents: list[tuple[str, str]],
    ...
    antigravity_discipline = f"""## Antigravity Execution Rules & Strict Gating

- **Prohibición Estricta de Truncamiento y Resúmenes (Anti-Lazy)**: Queda terminantemente prohibido usar comentarios como `// TODO`, `/* resto del código */` o resumir funciones y flujos. Todo archivo creado o modificado debe escribirse COMPLETO en disco usando las herramientas de edición.
- **Persistencia Física Obligatoria de Artefactos**: Queda prohibido simular compuertas en prosa. Cada fase DEBE persistir físicamente su archivo correspondiente en `docs/process/` antes de pasar a la siguiente:
  * **Fase 1 (`po-analyst`)**: Documento `docs/process/REQ-*.md` completo en disco.
  * **Fase 2 (`story-refiner`)**: Historias detalladas con AC verificables en `docs/process/stories/US-*.md`.
  * **Fase 2.5 (`test-engineer`)**: Plan de pruebas en `docs/process/test-plans/TEST-PLAN-*.md`.
  * **Fase 3 (`architect-reviewer`)**: Aprobación explícita y Schema Freeze verificado contra contratos/fixtures reales.
  * **Fase 4 (`backend/frontend-developer`)**: Implementación completa de código y migraciones sin placeholders.
  * **Fase 6 (`test-engineer`)**: Tests unitarios y de integración reales escritos y ejecutados (verde real).
  * **Fase 7 (`final-auditor`)**: Retrospectiva en `docs/process/RETRO-*.md` y respaldo con `kuraka-backup.py`.
- **Rigor Continuo Senior**: Mantener la profundidad de un Desarrollador Full-Stack Senior y Analista Funcional a lo largo de todos los turnos sin degradar a respuestas superficiales.

""" if target == "antigravity" else ""
...
''' if target == "codex" else ""}{antigravity_discipline}## How to work here — the 8-phase discipline
...
def _preamble(target: str) -> str:
    if target == "antigravity":
        return (
            ...
            "> **REGLAS DE EJECUCIÓN, TABLA VISUAL Y BLINDAJE DE CALIDAD:**\n"
            ...
            ">    ```\n"
            "> 2. **PROHIBICIÓN ESTRICTA DE TRUNCAMIENTO Y RESÚMENES (ANTI-LAZY)**: Queda terminantemente prohibido usar comentarios como `// TODO`, `/* resto del código */` o resumir funciones y flujos. Todo archivo creado o editado debe escribirse COMPLETO en disco con las herramientas de edición. Nunca simules compuertas en prosa.\n"
            "> 3. **PERSISTENCIA FÍSICA OBLIGATORIA POR FASE**: Antes de avanzar a la siguiente fase, el artefacto correspondiente (REQ en Fase 1, stories en Fase 2, test-plan en Fase 2.5, código en Fase 4, tests en Fase 6, RETRO en Fase 7) DEBE estar escrito físicamente en disco usando las herramientas de archivos. Si un archivo no existe en disco, el gate NO ha pasado.\n"
            "> 4. **EJECUCIÓN EN FOREGROUND — NO USAR BACKGROUND TASKS/SUBAGENTES**: Antigravity NO debe lanzar `manage_task`, `browser_subagent` ni comandos en background para ejecutar las fases de Kuraka. Todo el trabajo se realiza paso a paso en esta conversación activa.\n"
            "> 5. **ADOPCIÓN DE ROLES EN FOREGROUND**: Adoptá vos mismo el rol indicado por fase (`po-analyst` → `story-refiner` → `test-engineer` → `architect-reviewer` → `backend-developer`/`frontend-developer` → `code-reviewer` → `security-reviewer` → `test-engineer` → `final-auditor`).\n"
            "> 6. **MODO PASO A PASO VS MODO AUTOMÁTICO (AUTO-PILOT)**:\n"
